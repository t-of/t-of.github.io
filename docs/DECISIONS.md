...
### 散らばっていた未公開のアプリを tof/drafts/ にまとめる
`~/dev` と `~/GitHub` 直下にあったゲーム・アプリ 37 本を `~/GitHub/tof/drafts/` に移した。apps/ は公開中のものだけという決まりは変えない。公開版の古いコピー（dev/English、dev/square-game）はゴミ箱へ。ゼミ・研究・AtCoder などゲームでないものは元の場所のまま。オーナーの OK（2026-09-26）。
