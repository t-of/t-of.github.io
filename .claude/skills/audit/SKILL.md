...
1. `cd ~/GitHub/tof/t-of.github.io && npm run audit:browser` を実行する（時間がかかる。速く見るだけなら `npm run audit`）。
   各アプリのリポジトリが古いかもしれないので、先に `git -C ~/GitHub/tof/apps/<id> pull --ff-only` で最新にする（編集中の変更があるリポジトリは飛ばす）。
