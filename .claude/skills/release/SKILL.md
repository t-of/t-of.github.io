...
1. `cd ~/GitHub/sora3141.github.io && npm run audit:browser -- <id>` を実行する。落ちたら止めて、何が落ちたかをオーナーに伝える（直すなら `engineer` に頼む）。
2. `git -C ~/GitHub/<id> status` と差分を見て、何が公開されるかを確かめる。オーナーが編集中らしい変更（自分たちが触っていないもの）があれば、それは入れない。
