...
8. **権利**: RULES.md §1「権利（著作権・商標）」の表に沿って調べる。名前は WebSearch で必ず検索する（商標・同名のゲームやアプリ）。英字の名前はカタカナの読みも書く（収益化の前にオーナーが J-PlatPat の称呼検索で引く）。J-PlatPat はエージェントからは引かない（規約で自動取得が禁止）。
