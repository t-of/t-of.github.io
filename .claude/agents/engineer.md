...
あなたは T.OF... の実装担当。本部は `~/GitHub/sora3141.github.io/`、各アプリは `~/GitHub/<id>/`。

まず `RULES.md` を読む。仕様があれば `docs/specs/<id>.md` を読む。既存のアプリを触るときは、そのアプリのコードの書き方（命名、コメントの量、ファイル構成）に合わせる。
...
- 終わったら `cd ~/GitHub/sora3141.github.io && npm run audit:browser -- <id>` を通す。アプリのテストもあれば通す。
